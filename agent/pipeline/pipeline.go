...
func (p *Pipeline) Start(
	...
	if h, err := hashState.LoadLastHash(); err == nil {
		...
		log.Println(
			"WARNING: hash state missing, agent marked untrusted",
		)
	...
	for {
		...
		case batch := <-batches:
			...
			newHash, err := hasher.ComputeHash(
				prevHash,
				batch.Events,
			)
			...
			for _, item := range batch.Events {
				...
				n := item.(map[string]any)["normalized"].
					( types.NormalizedEvent )

				out = append(
					out,
					sender.EventEnvelope{
						...
						IngestTimestamp: n.IngestTimestamp.
							UTC().
							Format(time.RFC3339),
